...
function RoomDetails() {
 ...
 useEffect(() => {
  const fetchFeaturedRooms = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_URI}/rooms/${id}`);
 ...
 const roomDetails = {
   ...
   description: "Experience unparalleled luxury in our meticulously designed ocean-view suite. This spacious accommodation offers the perfect blend of comfort and sophistication, with breathtaking views and world-class amenities.",
   price: 200,
   size: "45 m²",
   ...
   view: "Ocean View",
   amenities: [
     { icon: "🛏️", name: "King-sized Bed", description: "Premium mattress and linens" },
     { icon: "📶", name: "High-speed WiFi", description: "Complimentary high-speed internet" },
     { icon: "🏊‍♂️", name: "Pool Access", description: "Exclusive infinity pool access" },
     { icon: "🍽️", name: "Room Service", description: "24/7 in-room dining" },
     { icon: "🚿", name: "Luxury Bathroom", description: "Rain shower and premium amenities" },
     { icon: "❄️", name: "Climate Control", description: "Individual temperature control" },
     { icon: "📺", name: "Entertainment", description: "55-inch Smart TV" },
     { icon: "☕", name: "Mini Bar", description: "Complimentary refreshments" }
   ]
 };

 const handleSubmit = async (e) => {
  e.preventDefault();
  const stripe = await loadStripe('pk_test_51QH8UhDVCApEbmXH0V05yN5isrK6roHA6ix4SyzpaxA1cWrfhPJSkWCI3fuTImeCVCgfjBCBkOn2i1ysZBov9vc500rvVtvsjX');
  const response = await fetch(`${process.env.REACT_APP_SERVER_URI}/payment`, {
      method: 'POST',
      headers: {
    ...
    {loading ? (
     ...
     <section className="rd-hero">
       <div className="rd-carousel">
         <div className="rd-slides" style={{ transform: `translateX(-${slideIndex * 100}%)` }}>
         ...
         <button className="rd-carousel-btn rd-prev" onClick={() => changeSlide(-1)}>
           ❮
         </button>
         <button className="rd-carousel-btn rd-next" onClick={() => changeSlide(1)}>
           ❯
         </button>
     ...
     <section className="rd-info-section">
       <div className="rd-info-container">
         <div className="rd-main-info">
           ...
           <p className="rd-description">{roomDetailsData.room_type.description}</p>
           ...
           <div className="rd-features">
             ...
             <div className="rd-feature">
               ...
               <span className="rd-feature-value">{roomDetailsData.capacity}</span>
             </div>
             <div className="rd-feature">
               <span className="rd-feature-icon">🛏️</span>
               <span className="rd-feature-label">Bed Type</span>
               <span className="rd-feature-value">{roomDetails.bedType}</span>
             </div>
             <div className="rd-feature">
               <span className="rd-feature-icon">🌅</span>
         ...
         <div className="rd-booking-card">
           <div className="rd-price-info">
             <span className="rd-price">${1000 || roomDetailsData.price.$numberDecimal}</span>
           ...
           <form className="rd-booking-form">
             ...
             <button type="submit" className="rd-book-now" onClick={handleSubmit}>
                Book Now
              </button>
           </form>
         </div>
